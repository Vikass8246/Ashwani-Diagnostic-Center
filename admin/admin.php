...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_user'])) {
    ...
    $name = escape($_POST['full_name']);
    $newUsername = escape($_POST['username']);
    $email = filter_var($_POST['email'], FILTER_SANITIZE_EMAIL);
    $password = password_hash($_POST['password'], PASSWORD_BCRYPT); // Hash the password

    // Prepare and bind the INSERT statement
    $insertQuery = "INSERT INTO register (full_name, username, email, password) VALUES (?, ?, ?, ?)";
    $stmt = $con->prepare($insertQuery);
    $stmt->bind_param("ssss", $name, $newUsername, $email, $password);
...
<body>
                ...
                <div class="d-flex">
                    ...
                    <div class="flex-grow-1 ms-2">
                        ...
                        <p class="fs-13 mb-0 text-muted"><i
                                class="mdi mdi-circle fs-10 align-middle text-primary me-1"></i>Users</p>
                    </div>
            ...
            <li class="list-group-item px-0 pb-0">
                <div class="d-flex">
                    ...
                    <div class="flex-grow-1 ms-2">
                        ...
                        <p class="fs-13 mb-0 text-muted"><i
                                class="mdi mdi-circle fs-10 align-middle text-success me-1"></i>Appointments</p>
                    </div>
                    <div class="flex-shrink-0 text-end">
                        <h6 class="mb-1">
                            <?php echo $totalAppointments; // Use the actual variable storing appointments count ?>
                        </h6>
                    </div>
                </div>
            </li><!-- end -->
    ...
    <div id="editPopup"
        style="display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: #fff; padding: 20px; border: 1px solid #ccc; z-index: 1000;">
    ...
    <section id="appointmentDetails" style="display: none;">
        ...
        if ($appointmentResult) {
            ...
            echo "<table border='1'>";
            echo "<thead><tr><th>ID</th><th>Name</th><th>Email</th><th>Phone</th><th>Appointment Date Time</th><th>Selected Tests</th><th>Total Price</th><th>File</th><th>Actions</th><th>Confirmation Status</th></tr></thead>";
            echo "<tbody>";
            foreach ($appointments as $appointment) {
                ...
                if ($appointment['file_content']) {
                    // Display preview button
                    echo "<a href='download.php?id={$appointment['id']}&action=preview' target='_blank'>Preview</a>";
                    // Display download button
                    echo "<a href='download.php?id={$appointment['id']}&action=download'>Download</a>";
                } else {
                    // Display blank if no file content
                ...
                if ($appointment['confirmed']) {
                    echo "<td id='confirmed{$appointment['id']}'>Confirmed</td>";
                } else {
                    echo "<td id='confirmed{$appointment['id']}'>Not Confirmed</td>";
                }
    ...
    `;
            ...
            function confirmAppointment($appointmentId) {
                // Update the database to mark the appointment as confirmed
            ...
            function confirmAppointment(appointmentId) {
                ...
                var isConfirmed = $('#confirmed' + appointmentId).text().trim() === 'Confirmed';

                if (isConfirmed) {
                    // If already confirmed, inform the user and exit
                    alert('This appointment is already confirmed.');
                    return;
                }

                // Ask for confirmation to proceed
                var confirmation = confirm(`Are you sure you want to confirm the appointment with ID: ${appointmentId}?`);
                if (confirmation) {
                    // Use AJAX to communicate with the server and confirm the appointment
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            alert(result.message); // Display confirmation message
                            if (result.success) {
                                // Send email notification to the user
                                sendEmailNotification(appointmentId);
                            }
                        }
                    });
                }
 ...
 // AJAX function to send email notification
function sendEmailNotification(appointmentId) {
    $.ajax({
        ...
        success: function (response) {
            // Check if the email was sent successfully based on the JSON response
            if (response.success) {
                // Email sent successfully
                alert(response.message);
                // Update confirmation status in the table
                $('#confirmed' + appointmentId).text('Confirmed');
            } else {
